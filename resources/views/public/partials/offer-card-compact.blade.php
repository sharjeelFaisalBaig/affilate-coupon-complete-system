...
<div class="flex flex-col rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
    ...
    <p class="mt-3 text-lg font-bold text-gray-900">{{ $offer->displayLabelFor($region) }}</p>
    ...
    <p class="mt-1 flex-1 text-sm text-gray-600">{{ $offer->title }} at {{ $store->name }}</p>

    @if ($offer->expiry_date)
        <p class="mt-1 text-xs text-gray-400">Expires {{ $offer->expiry_date->format('M j, Y') }}</p>
    @endif

    <a href="{{ route('public.store', [$region->code, $store->slug]) }}" class="mt-2 text-xs text-gray-700 underline hover:text-emerald-600">
    ...
    <div class="mt-3">
        @if ($isCoupon)
            <button type="button"
                    ...
                    class="block w-full rounded-md bg-emerald-500 px-4 py-2 text-center text-sm font-semibold text-white hover:bg-emerald-600">
                Shop with code
            </button>
        @else
            <a href="{{ $redirectUrl }}" target="_blank" rel="noopener sponsored"
               class="block w-full rounded-md bg-sky-600 px-4 py-2 text-center text-sm font-semibold text-white hover:bg-sky-700">
                View Deal
            </a>
        @endif
    </div>
